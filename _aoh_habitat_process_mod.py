...
def make_single_type_map(
    ...
    with yg.read_raster(habitat_path) as habitat_map:
        ...
        with VsimemFile(f"/vsimem/filtered_{habitat_value}.tif") as filter_map_path:
            ...
            if target_projection == "EPSG:4326" and pixel_scale is not None:
                bounds = (-180.0, -90.0, 180.0, 90.0)
            else:
                bounds = (
                    reprojected_area.left,
                    reprojected_area.bottom,
                    reprojected_area.right,
                    reprojected_area.top,
                )

            with VsimemFile(f"/vsimem/warped_{habitat_value}.tif") as warped_map_path:
                logger.info("Projecting %s...", habitat_value)

                # Build kwargs conditionally - never pass None for width/height/xRes/yRes,
                # since GDAL's own default is 0 and an explicit None gets stringified to
                # the literal "None" when building the warp command, which fails to parse.
                warp_kwargs = dict(
                    creationOptions=[],
                    multithread=True,
                ...
                if pixel_scale is not None:
                    warp_kwargs["xRes"] = pixel_scale
                    warp_kwargs["yRes"] = 0.0 - pixel_scale

                if width_height is not None:
                    warp_kwargs["width"] = width_height[0]
                    warp_kwargs["height"] = width_height[1]
...
def habitat_process(
    ...
    if pixel_scale is not None and width_height is not None:
        raise ValueError("Cannot specify both pixel_scale and width_height")
...
def main() -> None:
    ...
    print("Tom is lazy, take it up with him")
    # habitat_process(
    #     args.habitat_path,
    #     args.pixel_scale,
